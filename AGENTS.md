...
`~/.Codex/projects/-Users-tom-GitHub/memory/MEMORY.md`
...
- **Dev branch workflow** — `main` is always the live site (GitHub Pages). All feature work happens on `develop` and is merged to `main` only when stable and tested locally. Codex must always verify we are on `develop` before making any code changes, and must never commit or push to `main` directly.
...
## Working Principles
1. **Documentation stays current** — update READMEs and AGENTS.md every time something significant changes. Codex should flag AGENTS.md updates proactively during the session, not wait to be asked. Every session ends with a quick AGENTS.md review before stopping.
...
6. **Track decisions, not just code** — when we choose an approach (or rule one out), log the reasoning in AGENTS.md so future-us understands why.
...
11. **Hookify verification rule active** — `.Codex/hookify.require-verification.local.md` fires on every stop event. It requires running `source ~/.zshrc && npm test` and visually confirming UI changes before claiming work is done. Do not dismiss it.
